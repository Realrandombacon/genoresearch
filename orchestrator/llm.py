...
def build_system_prompt(context: str = "") -> dict:
    ...
    base = (
        ...
        "  - translate_sequence(filepath)  # DNA/RNA → protein translation (3 frames, best ORF)\n"
        "  - pubmed_search(query, max_results=5)  # search PubMed literature\n"
        "  - gene_info(gene_name)  # structured gene info (chromosome, summary, accessions)\n"
        "  - lab_train(config_name)\n"
        "  - lab_status()\n"
        "  - save_finding(title, description, evidence)\n"
        "  - list_findings()  # list all findings with index numbers\n"
        "  - read_finding(number)  # read full content of a finding by its number or title\n"
        "  - list_sequences()  # list all downloaded .fasta files — check before re-fetching!\n"
        "  - query_memory(question)\n"
        "  - my_stats()\n"
        "  - list_unexplored()\n"
        "  - note(text)  # save a research note to memory\n"
        "  - mark_explored(target, status)  # mark a gene/topic as explored ('partial'/'complete')\n"
        "  - mark_done(target)  # mark a target as fully exhausted\n"
        "  - dismiss(target, reason)  # dismiss a false lead\n\n"
        "Rules:\n"
        ...
        "  8. AUTONOMY — CRITICAL:\n"
        "     - You are FULLY AUTONOMOUS. There is NO human to ask questions to.\n"
        "     - NEVER say 'waiting for user', 'need clarification', or 'what do you want'.\n"
        "     - NEVER ask questions. ALWAYS decide and act.\n"
        "     - If unsure, make your best scientific judgment and proceed.\n"
        "     - If you hit a dead end, pivot to a new research angle.\n"
        "     - If data looks wrong, note it as a finding and move on.\n"
